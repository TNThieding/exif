...
from .delete_exif_baselines import (
...
class TestDeleteExif(unittest.TestCase):
    ...
    def test_delete_ascii_tags(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "image does not have attribute make"
        ):
            self.image.make
        with self.assertRaisesRegex(
            AttributeError, "image does not have attribute model"
        ):
            self.image.model
    ...
    def test_standard_delete(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "unknown image attribute dummy_attr"
        ):
